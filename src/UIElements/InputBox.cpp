...
InputBox::InputBox(int x, int y, int width, int height, int maxChar, MacroPad *macropad) : UIElement(x, y, width, height)
{
    this->strPos = 0;
    this->cursorPos = 0;
    this->maxChar = maxChar;
...
void InputBox::update()
{
    ...
    draw();
    this->cursor.draw();
    std::string str = macropad -> updateString();
