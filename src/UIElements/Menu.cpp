...
void Menu::clear(bool deleteSubElements, bool deleteLinkElements)
{
    this->menuSize = menuSize;
    this->menuPos = 0;
    this->cursorPos = 0;
...
Menu::~Menu()
{
    ...
    subElements.clear(); // 清空容器，确保没有悬空指针
    for (auto &linkElement : linkElements)
    {
        delete linkElement;
    }
    linkElements.clear(); // 清空容器，确保没有悬空指针
}
